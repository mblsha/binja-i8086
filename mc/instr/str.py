...
class InstrString(InstrHasWidth, Instruction):
    ...
    def _lift_inc_dec(self, il, regs, df_values):
        ...
        if df_values is not None and df_values.type == RegisterValueType.ConstantValue:
            if df_values.value:
                for reg in regs:
                    il.append(il.set_reg(2, reg, il.sub(2, il.reg(2, reg), il.const(2, self.width()))))
            else:
                for reg in regs:
                    il.append(il.set_reg(2, reg, il.add(2, il.reg(2, reg), il.const(2, self.width()))))
        else:
...
class Movs(InstrHasSegment, InstrString):
    ...
    def lift(self, il, addr, df_values=None):
        w = self.width()
        value = self._lift_mem_load(il, w, self._lift_phys_addr(il, self.segment(), 'si'))
        self._lift_mem_store(il, w, self._lift_phys_addr(il, 'es', 'di'), value)
...
class Cmps(InstrHasSegment, InstrString):
    ...
    def lift(self, il, addr, df_values=None):
        ...
        il.append(il.sub(
            w,
            self._lift_mem_load(il, w, self._lift_phys_addr(il, self.segment(), 'si')),
            self._lift_mem_load(il, w, self._lift_phys_addr(il, 'es', 'di')),
            '*',
        ))
        self._lift_inc_dec(il, ('si', 'di'), df_values)
...
class Stos(InstrString):
    ...
    def lift(self, il, addr, df_values=None):
        w = self.width()
        self._lift_mem_store(il, w, self._lift_phys_addr(il, 'es', 'di'), il.reg(w, 'ax'))
...
class Lods(InstrHasSegment, InstrString):
    ...
    def lift(self, il, addr, df_values=None):
        ...
        il.append(il.set_reg(
            w, 'ax', self._lift_mem_load(il, w, self._lift_phys_addr(il, self.segment(), 'si'))
        ))
